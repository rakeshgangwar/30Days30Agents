...
const aiSuggestionsService = {
  ...
  applyContentSuggestion: async (request: ApplySuggestionRequest): Promise<ApplySuggestionResponse> => {
    try {
      const response = await apiClient.post<ApplySuggestionResponse>("/content/apply-suggestion", request);
      return response.data;
    } catch (error) {
      console.error("Error applying suggestion:", error);
      // For now, return a mock response if the API endpoint doesn't exist yet
      return {
        improved_content: request.content // Return original content as fallback
      };
    }
  },
};
