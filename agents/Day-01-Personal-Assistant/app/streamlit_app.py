...
from config import init_user_preferences, USER_PREFERENCES_PATH
...
def main():
    ...
    if "memory" not in st.session_state:
        # Use LangGraph memory by default
        st.session_state.memory = LangGraphMemory()

    if "agent" not in st.session_state:
        # Create agent with the session ID as the thread ID
