...
import { FiClock, FiArrowLeft, FiExternalLink, FiCheckCircle } from 'react-icons/fi';
...
const LearningPathViewPage = () => {
  ...
  const handleTopicCompletion = async (topicIndex: number, isCompleted: boolean) => {
    console.log(`handleTopicCompletion called with topicIndex: ${topicIndex}, isCompleted: ${isCompleted}`);
    ...
    try {
      ...
      console.log(`Handling completion for topic: ${topic.name} (order: ${topic.order})`);
      ...
      console.log('Received updated learning path from API:', updatedLearningPath);
      ...
      <Container maxW="container.lg" py={8}>
        ...
        >
          <RouterLink to="/learning-paths">
            <Box as={FiArrowLeft} mr={2} display="inline-block" />
        ...
        <Alert.Root status="error" mb={6} borderRadius="md">
          ...
          <Alert.Description>{error || 'Learning path not found'}</Alert.Description>
        </Alert.Root>
      </Container>
    );
  }
  ...
  const progressPercentage = Math.round(
    (learningPath.progress.completed_topics / learningPath.progress.total_topics) * 100
  );
  ...
  return (
    <Container maxW="container.lg" py={8}>
      ...
      <Card.Root bg={cardBg} borderColor={borderColor} borderWidth="1px" borderRadius="lg" overflow="hidden" mb={6}>
        <Card.Header>
          <Heading as="h1" size="xl" color={headingColor}>{learningPath.title}</Heading>
          <Text mt={2} color={textColor}>{learningPath.description}</Text>
        </Card.Header>
      ...
      <Box mb={8}>
        <Heading as="h2" size="lg" mb={4}>Topics</Heading>
            ...
            <Accordion.Item key={index} value={index.toString()}>
              <Accordion.ItemTrigger>
                <Flex justify="space-between" align="center" width="100%" py={2}>
                  <HStack>
                    <Badge colorScheme="blue" mr={2}>{topic.order}</Badge>
                    <Text fontWeight="bold">{topic.name}</Text>
                  </HStack>
                  <Checkbox.Root
                    checked={
                      learningPath.progress.completed_topic_ids
                        ? (learningPath.progress.completed_topic_ids as number[]).includes(topic.order)
                ...
                <Stack gap={4}>
                  ...
                  {learningPath.resources && learningPath.resources.length > 0 && (
                    <Box>
                      ...
                      <List.Root gap={2}>
                        {(() => {
                          console.log(`Filtering resources for topic ${topic.name} (order: ${topic.order})`);

                          // First try to find resources specifically for this topic
                          let filteredResources = learningPath.resources
                            .filter(resource => {
                              const matches = resource.topic_order === topic.order ||
                                             (resource.topic && resource.topic.order === topic.order) ||
                                             (resource.topic_id === topic.order) ||
                                             (resource.topic_id === topic.order.toString()) ||
                                             (resource.topic_name === topic.name);
                              console.log(`Resource ${resource.name || resource.title}: matches=${matches}`, resource);
                              return matches;
                            });

                          console.log(`Found ${filteredResources.length} resources for topic ${topic.name}`);
                          ...
                          if (filteredResources.length === 0) {
                            ...
                            filteredResources = learningPath.resources
                              .filter(resource => {
                                if (Array.isArray(resource.topics)) {
                                  return resource.topics.some(t =>
                                    (typeof t === 'string' && t.toLowerCase() === topic.name.toLowerCase()) ||
                                    (typeof t === 'object' && t.name && t.name.toLowerCase() === topic.name.toLowerCase())
                                  );
                                }
                                return false;
                              });
                            console.log(`Found ${filteredResources.length} resources by matching topic name in topics array`);
                          }
                          ...
                          if (filteredResources.length === 0 && topic.order === 1) {
                            filteredResources = learningPath.resources
                              .filter(resource => {
                                const hasNoTopicAssociation = !resource.topic_order &&
                                                           !resource.topic &&
                                                           !resource.topic_id &&
                                                           !resource.topic_name;
                                return hasNoTopicAssociation;
                              });
                            console.log(`Found ${filteredResources.length} resources with no topic association`);
                          }
                              ...
                              <Card.Root variant="outline" size="sm">
                                <Card.Body>
                                  <Flex justify="space-between" align="center">
                                    <Box>
                                      <Text fontWeight="medium">{resource.name || resource.title}</Text>
                                    ...
                                    >
                                      <a href={resource.url} target="_blank" rel="noopener noreferrer">
                                        Open <Box as={FiExternalLink} ml={1} display="inline-block" />
                                      </a>
                                    </Button>
                                  </Flex>
                                </Card.Body>
